...
public class Bullet : MonoBehaviour
{
    ...
    [Tooltip("Cámara de juego. Si está vacío usa Camera.main")]
    ...
    {
        ActiveCount++;
        if (owner == Owner.Player) ActivePlayerCount++; else ActiveEnemyCount++;
    ...
    void OnDisable()
    {
        ...
        if (owner == Owner.Player) ActivePlayerCount = Mathf.Max(0, ActivePlayerCount - 1);
        else ActiveEnemyCount = Mathf.Max(0, ActiveEnemyCount - 1);
